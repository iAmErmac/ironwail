...
cvar_t vr_weapon_fire_xoffset = {"vr_weapon_fire_xoffset", "0", CVAR_ARCHIVE};
cvar_t vr_weapon_fire_yoffset = {"vr_weapon_fire_yoffset", "0", CVAR_ARCHIVE};
cvar_t vr_weapon_fire_zoffset = {"vr_weapon_fire_zoffset", "0", CVAR_ARCHIVE};
...
cvar_t vr_laser_deathmatch_allow = {"vr_laser_deathmatch_allow", "0", CVAR_NONE};
...
cvar_t vr_laser_beam_xoffset = {"vr_laser_beam_xoffset", "0", CVAR_ARCHIVE};
cvar_t vr_laser_beam_yoffset = {"vr_laser_beam_yoffset", "0", CVAR_ARCHIVE};
cvar_t vr_laser_beam_zoffset = {"vr_laser_beam_zoffset", "0", CVAR_ARCHIVE};
...
{
#if defined(ANDROID_GLES3)
    IW_Android_SetXRMultiviewRequested(var->value != 0.f);
#else
    if (vid_xr_backend)
...
{
#if defined(ANDROID_GLES3)
    return IW_Android_UsingXRMultiview();
#else
...
{
#if defined(ANDROID_GLES3)
    return IW_Android_BeginXRMultiview(fbo, width, height);
#else
...
{
#if defined(ANDROID_GLES3)
    return IW_Android_BeginXRMultiviewOverlayEye(eye, fbo, width, height);
#else
    return vid_xr_stereo_frame && IW_XRWin_BeginMultiviewOverlayEye(vid_xr_backend, eye, fbo, width, height);
#endif
}
...
#define num_readvars	Q_COUNTOF(read_vars)
    ...
    Cvar_RegisterVariable (&vr_weapon_fire_xoffset);
    Cvar_RegisterVariable (&vr_weapon_fire_yoffset);
    Cvar_RegisterVariable (&vr_weapon_fire_zoffset);
    Cvar_RegisterVariable (&vr_offhand_render_flip);
    Cvar_RegisterVariable (&vr_laser_sight);
    Cvar_RegisterVariable (&vr_laser_beam);
    Cvar_RegisterVariable (&vr_laser_deathmatch_allow);
    Cvar_RegisterVariable (&vr_laser_color);
    Cvar_RegisterVariable (&vr_laser_beam_width);
    Cvar_RegisterVariable (&vr_laser_beam_alpha);
    Cvar_RegisterVariable (&vr_laser_beam_xoffset);
    Cvar_RegisterVariable (&vr_laser_beam_yoffset);
    Cvar_RegisterVariable (&vr_laser_beam_zoffset);
...
putenv (vid_center);	/* SDL_putenv is problematic in versions <= 1.2.9 */
	...
	GL_SetupState ();
	#if defined(ANDROID_GLES3)
    IW_Android_SetXRMultiviewRequested(vr_multiview.value != 0.f);
#else
